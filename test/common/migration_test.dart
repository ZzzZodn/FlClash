...
void main() {
  group('Migration', () {
    ...
    test('adds the port card to a layout saved before it existed', () async {
      final configMap = _createConfigMap();
      final appSettingProps =
          configMap['appSettingProps']! as Map<String, Object?>;
      appSettingProps['dashboardWidgets'] = ['outboundMode', 'networkSpeed'];
      final store = _FakeMigrationStore(configMap: configMap, version: 1);

      final config = await Migration(store: store).run();

      // Appended, so a layout the user arranged keeps the order they chose.
    ...
    test('does not re-add a port card the user removed', () async {
      final configMap = _createConfigMap();
      final appSettingProps =
          configMap['appSettingProps']! as Map<String, Object?>;
      appSettingProps['dashboardWidgets'] = ['outboundMode'];
      final store = _FakeMigrationStore(
        configMap: configMap,
        version: Migration.currentVersion,
      );
